...
// 获取专业列表
const getMajorList = () => {
  if (majorStore.maj?.length) {
    majorOptions.value = majorStore.maj
  } else {
    majorStore.getMajorAll().then(() => {
    majorOptions.value = majorStore.maj as majorInfoData[]
    })
  }
}
//获取
...
const querySearchSchool = (queryString:any,cb:any)=>{
  ...
  let results = queryString?schoolOptions.value.filter(createFilter(queryString)):schoolOptions.value
  cb(results)
}
//专业提示
const querySearchMajor = (queryString:string,cb:any)=>{
  ...
  let resuls = queryString?majorOptions.value.filter(createFilter(queryString)):majorOptions.value
  cb(resuls)
}
...
// 上传组件的属性、用户上传的文件对象、上传的文件进行类型声明和类型检查
...
const rules=reactive<FormRules>({
  ...
  gener:[{required:true,message:"请输入性别",trigger:"blur"}],
