...
let validateOldPass=(rule:any,value:any,callback:any)=>{
  reqCheckPwd({userId:(useStore.user?._id as string),password:value}).then((reponse)=>{
    if(reponse.status===0){
      callback();
    }else{
...
const updatePwd=(fromEl:FormInstance|undefined)=>{
  if(!fromEl) return
  fromEl?.validate((valid)=>{
    if(valid){
      reqUpdatePwd({userId:(useStore.user?._id as string),password:userForm.pass}).then(reponse=>{
        if(reponse.status===0){
          handleLogout();
